...
$parserResultFile      = __DIR__ . '/../library/Parser.php';
$debugParserResultFile = __DIR__ . '/../library/Parser/Debug.php';
...
function resolveNodes($code) {
    return preg_replace_callback(
        ...
        function($matches) {
            ...
            return "new Parser\\Node\\" . $matches['name'] . '(' . $paramCode . '$attributes)';
        },
        $code
    );
}
    ...
    return preg_replace_callback(
        ...
        function($matches) {
            ...
            if ('error' == $name) {
                ...
                return 'throw new Parser\Error(' . $args[0] . ')';
            }
            ...
            if ('parseEncapsed' == $name) {
                ...
                return 'foreach (' . $args[0] . ' as &$s) { if (is_string($s)) { $s = Parser\Node\Scalar\String::parseEscapeSequences($s, ' . $args[1] . '); } }';
            }

            if ('parseEncapsedDoc' == $name) {
                assertArgs(1, $args, $name);

                return 'foreach (' . $args[0] . ' as &$s) { if (is_string($s)) { $s = Parser\Node\Scalar\String::parseEscapeSequences($s, null); } } $s = preg_replace(\'~(\r\n|\n|\r)$~\', \'\', $s); if (\'\' === $s) array_pop(' . $args[0] . ');';
            }
