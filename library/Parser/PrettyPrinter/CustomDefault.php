...
namespace PHP2C\Parser\PrettyPrinter;

use PHP2C\Parser,
	PHP2C\Parser\Node;

class CustomDefault extends Parser\PrettyPrinterAbstract
{
    // Special nodes
    ...
    public function pConst(Node\Constant $node) {
        return $node->name . ' = ' . $this->p($node->value);
    }

    // Names

    public function pName(Node\Name $node) {
        return implode('\\', $node->parts);
    }

    public function pName_FullyQualified(Node\Name\FullyQualified $node) {
        return '\\' . implode('\\', $node->parts);
    }

    public function pName_Relative(Node\Name\Relative $node) {
        return 'namespace\\' . implode('\\', $node->parts);
    }

    // Magic Constants

    public function pScalar_ClassConst(Node\Scalar\ClassConst $node) {
        return '__CLASS__';
    }

    public function pScalar_TraitConst(Node\Scalar\TraitConst $node) {
        return '__TRAIT__';
    }

    public function pScalar_DirConst(Node\Scalar\DirConst $node) {
        return '__DIR__';
    }

    public function pScalar_FileConst(Node\Scalar\FileConst $node) {
        return '__FILE__';
    }

    public function pScalar_FuncConst(Node\Scalar\FuncConst $node) {
        return '__FUNCTION__';
    }

    public function pScalar_LineConst(Node\Scalar\LineConst $node) {
        return '__LINE__';
    }

    public function pScalar_MethodConst(Node\Scalar\MethodConst $node) {
        return '__METHOD__';
    }

    public function pScalar_NSConst(Node\Scalar\NSConst $node) {
        return '__NAMESPACE__';
    }

    // Scalars

    public function pScalar_String(Node\Scalar\String $node) {
        return '\'' . $this->pNoIndent(addcslashes($node->value, '\'\\')) . '\'';
    }

    public function pScalar_Encapsed(Node\Scalar\Encapsed $node) {
    ...
    public function pScalar_DNumber(Node\Scalar\DNumber $node) {
    ...
    public function pExpr_Assign(Node\Expr\Assign $node) {
        return $this->pInfixOp('Expr_Assign', $node->var, ' = ', $node->expr);
    }

    public function pExpr_AssignRef(Node\Expr\AssignRef $node) {
    ...
    public function pExpr_AssignConcat(Node\Expr\AssignConcat $node) {
        return $this->pInfixOp('Expr_AssignConcat', $node->var, ' .= ', $node->expr);
    }

    public function pExpr_AssignMod(Node\Expr\AssignMod $node) {
